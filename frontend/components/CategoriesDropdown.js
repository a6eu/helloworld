                ...
                {!isTabletOrMobile ? (
                    ...
                    <>
                        <Transition
                            ...
                            leaveTo="opacity-0"
                        >
                            <Popover.Panel className="absolute w-full flex justify-center bg-white h-full left-0 top-0">
                                <div className={`w-full flex flex-col pt-4`}>
                                    <div className={`flex justify-end pr-4 pb-2`}><Image className="w-16 h-16" src={X} alt={"X"} onClick={() => close()}></Image></div>
                                    <ul className="w-full">
                                        {ctg.map((item, index) => (<li
                                            key={index}
                                            className={selectedCtg === index ? 'relative bg-white rounded-bl-2xl rounded-tl-2xl duration-500 scale-x-120' : ''}
                                        ...
                                        >
                                            <Disclosure >
                                                {({open}) => (
                                                    <>
                                                        <Disclosure.Button className={`flex w-full justify-between px-8`}>
                                                            <Link href={`/${item.categoryId}/`}
                                                                  onClick={() => {
                                                                      dispatch(setPath([item.name]));
                                                                      close();
                                                                  ...
                                                                  className={'text-[15px] font-normal cursor-default p-2 block text-black'}
                                                            >
                                                                {item.name}
                                                            </Link>
                                                            <ChevronUpIcon
                                                        ...
                                                        <Disclosure.Panel>
                                                            <ul className="text-left pl-14 mt-2">
                                                                {item.children.map((children) => (<li
                                                                    key={children.categoryId}
                                                                    className="text-[#606060] hover:text-[#1075B2] hover:cursor-pointer mb-1.5 ProductSansLight"
                                                                >
                                                                    <Link
                                                                        href={`/${ctg[selectedCtg].categoryId}/${item.categoryId}/${children.categoryId}`}
                                                                        onClick={() => dispatch(setPath([ctg[selectedCtg].name, item.name, children.name]))}>
                                                                        {children.name}
                                                                    </Link>
                                                                </li>))}
