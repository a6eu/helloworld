...
const SignUpForm = ({ onLogInClick }) => {
    ...
    const registration = async (e) => {
        ...
        if (!validateFirstName(firstName)) {
            setFirstNameError("Имя должно содержать только буквы.");
            isValid = false;
        }
        if (!validateSecondName(secondName)) {
            setSecondNameError("Фамилия должна содержать только буквы.");
            isValid = false;
        }
        if (!validatePhoneNumber(phoneNumber)) {
            setPhoneNumberError("Номер телефона должен содержать 10 цифр.");
            isValid = false;
        }
        ...
        if (!validateRepeatPassword(password, repeatPasswd)) {
            setRepeatPasswdError("Пароли не совпадают.");
            isValid = false;
        }
    ...
    return (
        <form className="w-full max-w-lg px-[50px] flex flex-wrap mb-6" onSubmit={registration}>
            <div className="flex w-full justify-between flex-wrap mb-3">
                ...
                <div className="w-[47%]">
                    ...
                    <input className="appearance-none block w-full bg-white text-gray-700 border border-[#1075B2] rounded py-1 text-s px-2.5 leading-tight focus:outline-none focus:bg-white focus:border-gray-500 hover:shadow-lg transition duration-500" onChange={(e) => setFirstName(e.target.value)} />
                ...
                <div className="w-[47%]">
                    ...
                    <input className="appearance-none block w-full bg-white text-gray-700 border border-[#1075B2] rounded py-1 text-s px-2.5 leading-tight focus:outline-none focus:bg-white focus:border-gray-500 hover:shadow-lg transition duration-500" onChange={(e) => setSecondName(e.target.value)} />
            ...
            <div className="w-full mb-3">
                ...
                <input className="appearance-none block w-full bg-white text-gray-700 border border-[#1075B2] rounded py-1 text-s px-2.5 leading-tight focus:outline-none focus:bg-white focus:border-gray-500 hover:shadow-lg transition duration-500" onChange={(e) => setPhoneNumber(e.target.value)} />
            ...
            <div className="w-full mb-3">
                ...
                <input className="appearance-none block w-full bg-white text-gray-700 border border-[#1075B2] rounded py-1 text-s px-2.5 leading-tight focus:outline-none focus:bg-white focus:border-gray-500 hover:shadow-lg transition duration-500" onChange={(e) => setEmail(e.target.value)} />
            ...
            <div className="w-full mb-7">
                ...
                <div className="relative">
                    ...
                    <div
                        className="cursor-pointer absolute -inset-y-2 right-0 flex rounded-none items-center px-2 text-gray-700">
                        {isPassword ? (
                            <svg
            ...
            <div className="w-full mb-3">
                <label className="select-none block uppercase tracking-wide text-gray-700 text-[10px] font-bold mb-1">
                    Подтвердите пароль
                </label>
                <input type="password"
                       className="appearance-none block w-full bg-white text-gray-700 border border-[#1075B2] rounded py-1 px-2.5 leading-tight focus:outline-none focus:bg-white focus:border-gray-500 hover:shadow-lg transition duration-500"
